...
export class AuthService {
  ...
  async validateUser(
    ...
    try {
      ...
      if (organizationId) {
        ...
        user = users.find((u) => u.email === email && u.hasRole(RoleName.SUPER_ADMIN)) || null;
      }
  ...
  async refreshToken(refreshTokenString: string, ipAddress?: string): Promise<AuthResponse> {
    try {
      const tokens = await this.tokenService.refreshAccessToken(refreshTokenString, ipAddress);
